# Implement: smart-collection-row-control

> 작성 시점과 실행 시점 사이 코드 변경 가능성. 위치는 항상 grep으로 재확인 후 변경.

## 목표
- spec(`plans/spec/research/smart-collection-row-control.md`)의 핵심 동작: 스마트 컬렉션 행 클릭 시 해당 컬렉션 선택 및 규칙 기반 논문 목록 필터링.
- design(`plans/design/research/smart-collection-row-control.md`)의 Component breakdown / States를 코드로 옮긴다.
...
8. `plans/test/research/smart-collection-row-control.md`가 있으면 거기 명시된 테스트 실행하여 통과 확인.