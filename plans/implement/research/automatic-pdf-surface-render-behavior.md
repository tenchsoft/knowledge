# Implement: automatic-pdf-surface-render-behavior

> 작성 시점과 실행 시점 사이 코드 변경 가능성. 위치는 항상 grep으로 재확인 후 변경.

## 목표
- spec(`plans/spec/research/automatic-pdf-surface-render-behavior.md`)의 핵심 동작: PDF 모드에서 현재 페이지/줌/회전/이미지 데이터로 PDF 표면 자동 렌더.
...
8. `plans/test/research/automatic-pdf-surface-render-behavior.md`가 있으면 거기 명시된 테스트 실행하여 통과 확인.