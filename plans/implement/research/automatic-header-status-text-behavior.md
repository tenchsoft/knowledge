# Implement: automatic-header-status-text-behavior

> 작성 시점과 실행 시점 사이 코드 변경 가능성. 위치는 항상 grep으로 재확인 후 변경.

## 목표
- spec(`plans/spec/research/automatic-header-status-text-behavior.md`)의 핵심 동작: 헤더에 import_status, reader_mode, favorites 상태 텍스트 자동 표시.
...
8. `plans/test/research/automatic-header-status-text-behavior.md`가 있으면 거기 명시된 테스트 실행하여 통과 확인.