# Implement: pdf-search-enter-control

> 작성 시점과 실행 시점 사이 코드 변경 가능성. 위치는 항상 grep으로 재확인 후 변경.

## 목표
- spec(`plans/spec/research/pdf-search-enter-control.md`)의 핵심 동작: PDF 검색 포커스 상태에서 Enter 시 다음 검색 결과로 이동.
- design(`plans/design/research/pdf-search-enter-control.md`)의 Component breakdown / States를 코드로 옮긴다.
...
8. `plans/test/research/pdf-search-enter-control.md`가 있으면 거기 명시된 테스트 실행하여 통과 확인.