# Implement: advanced-search-year-range-field

> 작성 시점과 실행 시점 사이 코드 변경 가능성. 위치는 항상 grep으로 재확인 후 변경.

## 목표
- spec(`plans/spec/research/advanced-search-year-range-field.md`)의 핵심 동작: 고급 검색 패널 Year range 필드 타이핑 시 year_from/year_to 파싱 및 논문 목록 연도 범위 필터링.
- design(`plans/design/research/advanced-search-year-range-field.md`)의 Component breakdown / States를 코드로 옮긴다.
...
|----------|------|-------|----------|
| `research.advanced.year` | `TextInput` | `Year` | show_advanced_search=true |
...
8. `plans/test/research/advanced-search-year-range-field.md`가 있으면 거기 명시된 테스트 실행하여 통과 확인.