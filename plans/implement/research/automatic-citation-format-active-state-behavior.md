# Implement: automatic-citation-format-active-state-behavior

> 작성 시점과 실행 시점 사이 코드 변경 가능성. 위치는 항상 grep으로 재확인 후 변경.

## 목표
- spec(`plans/spec/research/automatic-citation-format-active-state-behavior.md`)의 핵심 동작: citation_export_format 상태에 따라 인용 포맷 버튼 활성 상태 자동 반영.
...
8. `plans/test/research/automatic-citation-format-active-state-behavior.md`가 있으면 거기 명시된 테스트 실행하여 통과 확인.