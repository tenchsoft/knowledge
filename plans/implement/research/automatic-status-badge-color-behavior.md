# Implement: automatic-status-badge-color-behavior

> 작성 시점과 실행 시점 사이 코드 변경 가능성. 위치는 항상 grep으로 재확인 후 변경.

## 목표
- spec(`plans/spec/research/automatic-status-badge-color-behavior.md`)의 핵심 동작: ReadingStatus에 따라 상태 배지 색상 자동 선택.
...
8. `plans/test/research/automatic-status-badge-color-behavior.md`가 있으면 거기 명시된 테스트 실행하여 통과 확인.