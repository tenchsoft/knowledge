...
#[test]
fn new_concept_authoring_button_appends_concept_ui_e2e() {
    let mut harness = harness();
    complete_profile_setup(&mut harness);

    let panel = open_authoring_panel(&mut harness);
    decode_png(&panel);
    assert_selector(&panel, "study.authoring.new_concept");

    // Type something in the title first
    click(&mut harness, "study.authoring.title");
    type_text(&mut harness, "Concept Title");

    // Verify title was typed
    let after_type = harness.automation_capture(UiAutomationCaptureRequest::default());
    let title_after_type = find_node(tree(&after_type), &selector("study.authoring.title"))
        .expect("title node after typing");
    assert_eq!(
        title_after_type.label.as_deref(),
        Some("Concept Title"),
        "title should contain typed text before new concept click"
    );

    // Click New Concept button
    let after_new_concept = click(&mut harness, "study.authoring.new_concept");
    decode_png(&after_new_concept);

    // Title should be reset to empty
    let title_node = find_node(tree(&after_new_concept), &selector("study.authoring.title"))
        .expect("title node");
    assert_eq!(
        title_node.label.as_deref(),
        Some(""),
        "title should be reset to empty after new concept"
    );

    // Panel should still be open
    assert_selector(&after_new_concept, "study.authoring.panel");
}

#[test]
fn new_concept_authoring_button_repeated_clicks_ui_e2e() {
    let mut harness = harness();
    complete_profile_setup(&mut harness);
    ...
    // Click New Concept multiple times
    let first = click(&mut harness, "study.authoring.new_concept");
    decode_png(&first);
    ...
    let second = click(&mut harness, "study.authoring.new_concept");
    decode_png(&second);
    let title_node =
    ...
    assert_eq!(
        title_node.label.as_deref(),
        Some(""),
        "title should be reset again after second new concept click"
    );
