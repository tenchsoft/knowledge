...
#[test]
fn new_curriculum_authoring_button_creates_draft_ui_e2e() {
    let mut harness = harness();
    complete_profile_setup(&mut harness);

    let panel = open_authoring_panel(&mut harness);
    decode_png(&panel);
    assert_selector(&panel, "study.authoring.new_curriculum");

    // Type something in the title and body first
    click(&mut harness, "study.authoring.title");
    type_text(&mut harness, "Old Title");
    click(&mut harness, "study.authoring.body");
    type_text(&mut harness, "Old Body");

    // Click New Curriculum button
    let after_new = click(&mut harness, "study.authoring.new_curriculum");
    decode_png(&after_new);

    // Title and body should be reset to empty
    let title_node =
        find_node(tree(&after_new), &selector("study.authoring.title")).expect("title node");
    assert_eq!(
        title_node.label.as_deref(),
        Some(""),
        "title should be reset to empty after new curriculum"
    );
    let body_node =
        find_node(tree(&after_new), &selector("study.authoring.body")).expect("body node");
    assert_eq!(
        body_node.label.as_deref(),
        Some(""),
        "body should be reset to empty after new curriculum"
    );

    // Panel should still be open
    assert_selector(&after_new, "study.authoring.panel");
}

#[test]
fn new_curriculum_authoring_button_repeated_clicks_ui_e2e() {
    let mut harness = harness();
    complete_profile_setup(&mut harness);
    ...
    // Click New Curriculum multiple times
    let first = click(&mut harness, "study.authoring.new_curriculum");
    decode_png(&first);
    ...
    let second = click(&mut harness, "study.authoring.new_curriculum");
    decode_png(&second);
    let title_node =
    ...
    assert_eq!(
        title_node.label.as_deref(),
        Some(""),
        "title should be reset again after second new curriculum click"
    );
